...
## Download wpn core
sudo -u apache wp core --path="${sitepath}" --locale=en_US || exit 1;

## Create the config file
sudo -u apache wp config create --dbhost=${my_dbhost}:${my_dbport} --dbuser=${my_dbsu} -dbpass=${my_dbsu_pass} --dbname=wpn_${site}_${env_name} --locale=en_US || exit 1;

## Create the database
sudo -u apache bash -c "mysql -h ${my_dbhost} -P ${my_dbport} -u ${my_dbsu} -p${my_dbsu_pass} -e 'create database wpn_${site}_${env_name}; GRANT ALL PRIVILEGES ON wpn_${site}_${env_name}.* TO ${my_dbsu} IDENTIFIED BY ${my_dbsu_pass}'" || exit 1;

## Install wpn core
sudo -u apache wp core install --url="${site}.${my_host_suffix}${subpath}" --title="${site}" --admin_user="admin@${site}.${my_host_suffix}${subpath}" --admin_email=${site}.${my_host_suffix} || exit 1;

## Apply the apache config
wpn_httpd_conf.sh "$sitepath" "$siteTYPE" || exit 1;
